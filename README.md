...
10. Embiggen automation to upscale-cut-img2img-stitch and achieve high
res and more prompt-specific detail without extra VRAM.
...
Unlike Img2Img, only the size of the tiles used to Embiggen the image are the `--width` (`-W`) and `--height` (`-H`).

The scaling factor is how much larger than the `--init_img` the output should be, and will
multiply both x and y axis, so an image that is a scaling factor of 3.0 has 3*3= 9 times
as many pixels, and will take (at least) 9 times as long (see overlap for why it might be 
...
If there weren't enough clouds in the sky of that forest you just made (and that image
