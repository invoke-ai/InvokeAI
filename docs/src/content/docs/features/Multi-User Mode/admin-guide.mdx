...
The download queue (`/api/v1/download_queue`) is likewise administrator-only.
Its downloads are written to a separate sibling directory beside the configured
`download_cache_dir`; this keeps arbitrary administrator-selected downloads out
of the trusted model cache.
...
For security, the guarded download queue ignores `HTTP_PROXY`, `HTTPS_PROXY`,
and `ALL_PROXY` environment settings. A proxy resolves destinations outside
InvokeAI, so its socket cannot be checked against the public-address policy.
Use direct outbound access for guarded downloads, or explicitly enable private
download URLs only when the proxy's destination policy is trusted.
