...
The fastest way to generate video is the **Video panel** in the editor's left rail (next to Generate and Upscale) — no workflow required. Type a prompt and pick a Wan 2.2 or MiniMax H3 model for text-to-video; add a **First Frame** for image-to-video, a **Last Frame** for first-to-last interpolation, or an **Initial Video** (with trim) to extend an existing clip toward an optional destination image. The panel knows each model family's constraints — frame grids, canvas rules, fixed vs. adjustable frame rates, which conditioning modes exist per variant — and derives legal output dimensions from your aspect-ratio preset or the conditioning media automatically. When the matching distillation LoRA is installed (Wan's Lightning pair, H3's Turbo), the panel's **fast path** is on by default at the distilled step counts; switch it off for full-quality sampling. The bundled workflow templates below remain available for advanced graph-level control.
...
Start with the [**Video Workflows** guide](/features/video-workflows/) — a plain-language tour of the ten ready-made workflows that ship with InvokeAI and how to run them. This page is the deeper technical reference behind them.
:::
...
The shipped starter workflows ("Text to Video - Wan 2.2 Lightning", "Image to Video - Wan 2.2 Lightning") are the easiest starting point — load them from the workflow library, pick your models, set a prompt, and Invoke. The [Video Workflows guide](/features/video-workflows/) walks through all ten and how to choose between them; the sections below describe what's happening inside so you can build your own.
