...
<Card title="Workflow Image Export" icon="download">
  Click the camera button in the workflow editor controls to download a 2x PNG of the complete workflow. The image includes connectors, the background grid, and the selected Invoke color scheme, with nodes shown deselected and the MiniMap omitted.
</Card>
