...
{/* SCREENSHOT PLACEHOLDER: video-panel-overview.png — the full Video panel in the left rail with a Wan A14B model selected, showing the Prompt, Start & End Images, Initial Video, Dimensions, and Model sections. */}
...
{/* SCREENSHOT PLACEHOLDER: video-panel-start-end-images.png — the Start & End Images section with a first-frame image set, showing the Initial Video section below it in its disabled state. */}
...
{/* SCREENSHOT PLACEHOLDER: video-panel-initial-video.png — the Initial Video section with a clip loaded, showing the trim sliders and the start/end bound thumbnails. */}
...
{/* SCREENSHOT PLACEHOLDER: video-panel-model-section.png — the Model section with a Wan A14B model selected and the Lightning toggle on, showing Steps 4 and both CFG fields. */}
...
{/* SCREENSHOT PLACEHOLDER: video-panel-expert-advisory.png — the Model Components section showing the expert-wiring advisory badge with its Swap experts button. */}
...
H3 is the heaviest local video option — plan on a high-VRAM GPU (24 GB+) with `enable_partial_loading` and generous system RAM for offloaded weights.
