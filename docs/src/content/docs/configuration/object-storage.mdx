...
When `image_subfolder_strategy` is configured (e.g. `date`, `type`, or
`hash`), the S3 backend mirrors the on-disk layout and inserts the same
subfolder into both prefixes, e.g. `images/<subfolder>/<image-name>.png`
and `thumbnails/<subfolder>/<image-name>.webp`.
