...
Developers with a CUDA or ROCm device can validate Wan VAE memory estimates against measured peak reserved memory with `python scripts/calibrate_wan_vae_working_memory.py --vae <directory-or-safetensors-file>`. The script reports the estimate, measured peak, and implied scaling constant for the selected VAE and clip shape. Use `--tiling` to measure the spatially tiled full-decode fallback; it overrides streaming mode. Use `--tile-size <pixels>` to override the VAE's default tile size.
