...
Developers with a CUDA or ROCm device can validate Wan VAE memory estimates against measured peak reserved memory with `python scripts/calibrate_wan_vae_working_memory.py --vae <path>`. The script reports the estimate, measured peak, and implied scaling constant for the selected VAE and clip shape.
