...
:::note
This page describes **version 1** of the `.invk` container, which is canvas-only and written by this frontend. Workbench projects write version 2 of the same container — see [Project Files (.invk)](/development/front-end/project-files/). The two readers each pin their own version and refuse the other's archives.
:::
...
Future format changes should increment `CANVAS_PROJECT_VERSION` and keep validation in `parseManifest()` explicit so unsupported project files fail early. Version `2` is taken by the Workbench project archive.
