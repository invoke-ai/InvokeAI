...
`.invk` is shared with the previous frontend's [canvas projects](/development/front-end/canvas-projects/), which write version 1 of the same container. Workbench projects write version 3. The two are deliberately not interchangeable — see [Versions](#versions).
...
`.invk` files are ZIP archives. The current manifest version is `3`.
...
| `board.json`          | v3       | The project board's visible contents as the exporting server enumerated them: `{version: 1, items: [{kind, category, name, starred}]}`.  |
...
Version 3 requires `board.json` and refuses an archive without it as `damaged`. Version 2 is still read and is deliberately *not* asked to carry board membership: an archive written before boards existed has no board to describe, and inventing an empty one for it would claim knowledge the file does not contain — so a v2 import restores its document references and lets the server create the new project an empty board. The archive's `board.json` carries its own `version: 1`, which is the *file's* version and not the API's: the snapshot endpoint returns `{items}` and nothing else, so neither format is pinned to the other.
...
Future format changes should add a new member to the union rather than widening version 3, so that every version this app has ever written can be named precisely when refused.
