...
The input `File.size` is rejected before `arrayBuffer()` allocates its bytes. On export, the accumulated entries are checked before packing and the packed ZIP is checked again afterward, because compression can change the final output size.
