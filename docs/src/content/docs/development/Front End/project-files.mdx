...
**The gallery selection is excluded, of either kind.** `selectedImage`, `selectedImageName` and `selectedImageNames` are pointers into per-install gallery content, not things the document renders; a project arriving on another machine should open with nothing selected rather than drag a stranger's gallery in behind it. The skip is by key on purpose: `GalleryItem` happens to spell its name field `name`, which no collector key matches, so these would be missed anyway — but that is an accident of naming, and the explicit skip is what stops a future rename from silently changing what a project file contains.
