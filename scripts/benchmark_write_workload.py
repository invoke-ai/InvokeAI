...
def _insert_range(
    adapter: EngineAdapter, start: int, count: int, per_row_latencies_ms: list[float] | None = None
) -> None:
...
def write_csvs(
    out_dir: Path, single: list[SingleThreadResult], concurrent: list[ConcurrentResult], mixed: list[MixedResult]
) -> None:
    ...
    with (out_dir / "write_concurrent.csv").open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(
            ["engine", "workers", "rows_per_worker", "total_rows", "wall_s", "rows_per_s", "p50_ms", "p95_ms", "p99_ms"]
        )
        for r in concurrent:
            w.writerow(
                [
...
def write_markdown(
    ...
    for r in single:
        lines.append(
            f"| {r.engine} | {r.total_s:.2f} s | {r.rows_per_s:,.0f} "
    ...
    lines.append("")
    lines.append(
        f"K workers, each with its own connection, each inserting "
    ...
    for r in concurrent:
        lines.append(
            f"| {r.engine} | {r.workers} | {r.total_rows} | {r.wall_s:.2f} "
            f"| {r.rows_per_s:,.0f} | {r.p50_ms:.2f} | {r.p95_ms:.2f} | {r.p99_ms:.2f} |"
        )
    lines.append("")
    lines.append(
        "`p50/p95/p99` are **per-INSERT** latencies. If they shoot up with more workers, "
    ...
    lines.append("")
    lines.append(
        f"Table seeded with {mixed_seed_rows} rows. One continuous writer thread + "
    ...
    for r in mixed:
        lines.append(
            f"| {r.engine} | {r.readers} | {r.writer_rows_per_s:,.0f} | {r.reader_calls} "
...
def main() -> None:
    ...
    ap.add_argument("--sqlite-path", type=Path, default=None, help="SQLite file path (default: tmp file).")
    ...
    print(
        f"[write-bench] rows_per_test={args.rows_per_test}  workers={args.workers}  "
    ...
    for ad in adapters:
        ...
        print(
            f"    total={r.total_s:.2f}s  {r.rows_per_s:,.0f} rows/s  "
    ...
    for w in args.workers:
        ...
        for ad in adapters:
            ...
            print(
                f"    wall={r.wall_s:.2f}s  {r.rows_per_s:,.0f} rows/s  "
                f"p50={r.p50_ms:.2f} p95={r.p95_ms:.2f} p99={r.p99_ms:.2f}"
            )
    ...
    for w in args.workers:
        for ad in adapters:
            print(f"  [{ad.name}] readers={w}  duration={args.mixed_duration_s}s  seed={args.mixed_seed_rows}")
            r = run_mixed_workload(ad, w, args.mixed_duration_s, args.mixed_seed_rows)
            mixed_results.append(r)
            print(
                f"    writer={r.writer_rows_per_s:,.0f} rows/s  "
    ...
    write_markdown(
        ...
        args.rows_per_test,
        args.workers,
        args.mixed_duration_s,
        args.mixed_seed_rows,
    )
