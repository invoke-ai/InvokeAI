...
Run on a CUDA or ROCm device with a local diffusers VAE directory:

    python scripts/calibrate_wan_vae_working_memory.py --vae /path/to/vae
...
def _load_vae(path: Path, dtype: torch.dtype) -> AutoencoderKLWan:
    if not path.is_dir():
        raise ValueError("--vae must point to a diffusers AutoencoderKLWan directory")
    return AutoencoderKLWan.from_pretrained(path, local_files_only=True, torch_dtype=dtype)
...
def main() -> None:
    ...
    parser.add_argument("--vae", type=Path, required=True, help="Diffusers AutoencoderKLWan directory.")
