...
class ObjectSerializerDisk(ObjectSerializerBase[T]):
    ...
    def load(self, name: str) -> T:
        ...
        try:
            # `weights_only=True` is the default from torch 2.6 onwards, but pin it explicitly: these files are
            # named by (untrusted) graph input, so an unpickling path must never be reachable from a graph.
            return torch.load(file_path, weights_only=True)  # pyright: ignore [reportUnknownMemberType]
    ...
    def _validate_name(name: str) -> None:
        """Validates that an object name is a plain filename, so it cannot escape the output directory.

        Names are generated by `_new_name()`, but they travel to the client inside node outputs (e.g.
        `TensorField.tensor_name`) and come back as node inputs, so they are untrusted on the way in. Anything
        other than a bare filename - a separator, a drive letter, `..`, a leading `/` - would make
        `self._output_dir / name` resolve outside the store.

        :raises InvalidObjectNameError: if the name is not a plain filename
        """
        # `PureWindowsPath` treats both `/` and `\` as separators and understands drive letters, so it rejects
        # Windows-shaped payloads even when we are running on posix.
        if not name or name in (".", "..") or "\x00" in name or PureWindowsPath(name).name != name:
            raise InvalidObjectNameError(name)
