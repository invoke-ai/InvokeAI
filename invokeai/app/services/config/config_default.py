...
class InvokeAIAppConfig(BaseSettings):
    ...
    Attributes:
        ...
        smart_partial_loading: EXPERIMENTAL (off by default, pending validation across hardware). Estimate each denoise operation's working-memory need (scaled by the model and output resolution) and reserve only that much VRAM, instead of always holding back the flat device_working_mem_gb. This keeps more of the model resident in VRAM on memory-constrained cards (faster). Operations that do not provide an estimate keep the full flat reserve. Enable to opt in.
    ...
    smart_partial_loading:         bool = Field(default=False,              description="EXPERIMENTAL (off by default, pending validation across hardware). Estimate each denoise operation's working-memory need (scaled by the model and output resolution) and reserve only that much VRAM, instead of always holding back the flat device_working_mem_gb. This keeps more of the model resident in VRAM on memory-constrained cards (faster). Operations that do not provide an estimate keep the full flat reserve. Enable to opt in.")
