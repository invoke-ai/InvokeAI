...
class InvokeAIAppConfig(BaseSettings):
    ...
    Attributes:
        ...
        model_cache_keep_alive: How long to keep models in cache after last use, in minutes. A value of 0 means models are kept in cache indefinitely. If no model generations occur within the timeout period, the model cache is cleared using the same logic as the 'Clear Model Cache' button.
