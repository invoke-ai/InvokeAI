...
class InvokeAIAppConfig(BaseSettings):
    ...
    Attributes:
        ...
        smart_partial_loading: Estimate each denoise operation's working-memory need (scaled by the model and output resolution) and reserve only that much VRAM, instead of always holding back the flat device_working_mem_gb. This keeps more of the model resident in VRAM on memory-constrained cards (faster), while still reserving the full amount for operations that do not provide an estimate. Disable to revert to the flat reserve.
    ...
    smart_partial_loading:         bool = Field(default=True,               description="Estimate each denoise operation's working-memory need (scaled by the model and output resolution) and reserve only that much VRAM, instead of always holding back the flat device_working_mem_gb. This keeps more of the model resident in VRAM on memory-constrained cards (faster), while still reserving the full amount for operations that do not provide an estimate. Disable to revert to the flat reserve.")
