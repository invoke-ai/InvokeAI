"""Reference-image (VAE-latent) encoder for Wan 2.2 image-to-video conditioning.
...
- **A14B** (16-channel Wan 2.1 VAE): a 20-channel condition tensor (4-ch
  ...
  to the noise latents along the channel dim each step.
- **TI2V-5B** (48-channel Wan 2.2 VAE): a single-frame 48-channel condition
  that the denoise loop blends with the noisy latents at every step
  (expand_timesteps path).

Supports both single-frame (image I2V, ``num_frames=1``) and multi-frame
(video I2V, e.g. ``num_frames=81``) condition tensors; multi-frame and FLF2V
(end-image interpolation) are A14B-only.
"""
