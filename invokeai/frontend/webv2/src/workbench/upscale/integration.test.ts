...
import { createQueueItemBackendSubmission, getQueueItemResultImageOptions } from '@workbench/WorkbenchRuntime';
...
describe('Upscale snapshot submission', () => {
  it('captures a resolved immutable snapshot, prompt history, graph preview, and generate-style backend request', () => {
    ...
    expect(getQueueItemResultImageOptions(queueItem)).toEqual({ resultNodeIds: ['upscale_output'] });
    expect(createQueueItemBackendSubmission(project, queueItem)).toMatchObject({
      kind: 'generate',
      request: {
        batchCount: 3,
        negativePrompt: 'blur',
