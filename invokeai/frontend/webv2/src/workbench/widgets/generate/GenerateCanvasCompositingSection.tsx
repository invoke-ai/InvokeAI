...
import { Field, Select } from '@workbench/components/ui';
import {
...
export const GenerateCanvasCompositingSection = () => {
  const sourceId = useActiveProjectSelector((project) => project.invocation.sourceId);
  if (sourceId !== 'canvas') {
    return null;
  }
  return <CanvasCompositingControls />;
};
...
const CanvasCompositingControls = () => {
  ...
  const patch = useCallback(
    (partial: Record<string, unknown>) => {
      dispatch({ type: 'patchWidgetValues', values: partial, widgetId: 'canvas' });
    },
    [dispatch]
  );
  ...
  return (
    <GenerateCollapsibleSection label={t('widgets.generate.compositing')}>
      <Stack gap="2" p="2">
