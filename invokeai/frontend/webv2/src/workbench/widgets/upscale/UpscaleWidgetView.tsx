...
import type { GenerateLora, MainModelConfig, VaePrecision } from '@workbench/generation/types';
import type { ModelConfig } from '@workbench/models/types';
import type { PromptHistoryItem } from '@workbench/types';
import type { UpscaleWidgetValues } from '@workbench/upscale/types';
...
import { Button, Combobox, Field, IconButton, Slider, Tooltip } from '@workbench/components/ui';
import { toaster } from '@workbench/components/ui/toaster';
import { getGalleryImagesByNames, getImageThumbnailUrl, uploadGalleryImage } from '@workbench/gallery/api';
...
} from '@workbench/generation/projectPromptDraft';
import {
...
} from '@workbench/upscale/settings';
import { isGalleryImageDragData } from '@workbench/widgets/gallery/galleryDnd';
import { NegativePromptField } from '@workbench/widgets/generate/promptFields/NegativePromptField';
import { PositivePromptField } from '@workbench/widgets/generate/promptFields/PositivePromptField';
...
const UpscaleModelReconciler = ({
  ...
  useMountEffect(() => {
    ...
    dispatch({ projectId, type: 'patchWidgetValues', values: { ...values }, widgetId: 'upscale' });
  });
...
const UpscaleImageField = ({
  ...
  const setGalleryImage = useCallback(
    async (imageName: string) => {
      ...
      try {
        const [image] = await getGalleryImagesByNames([imageName]);
        ...
        dispatch({ area: 'upscale', message, namespace: 'generation', type: 'recordError' });
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) {
        ...
        dispatch({
          area: 'upscale',
          message: t('widgets.upscale.unsupportedFile'),
      ...
      try {
        const image = await uploadGalleryImage(file, 'none');
          ...
          <HStack align="stretch" gap="3" h="24" p="2">
            <Box bg="blackAlpha.300" boxSize="20" flexShrink="0" overflow="hidden" rounded="sm">
              <Image
                ...
                outlineOffset="-1px"
                rounded="sm"
                src={getImageThumbnailUrl(inputImage.image_name)}
...
export const UpscaleWidgetView = () => {
  ...
  const selection = useActiveProjectSelector(
    (project) => {
      const instance = Object.values(project.widgetInstances).find((candidate) => candidate.typeId === 'upscale');

      return {
        promptDraft: getProjectPromptDraft(project),
        projectId: project.id,
        rawValues: instance?.state.values ?? {},
        showPromptSyntaxHighlighting: project.settings.showPromptSyntaxHighlighting,
      };
    },
    (left, right) =>
  ...
  const patch = useCallback(
    (next: Partial<UpscaleWidgetValues>) =>
      dispatch({ projectId: selection.projectId, type: 'patchWidgetValues', values: next, widgetId: 'upscale' }),
    [dispatch, selection.projectId]
  );
  const patchPromptDraft = useCallback(
    (next: ProjectPromptDraftPatch) =>
      dispatch({ projectId: selection.projectId, type: 'patchProjectPromptDraft', values: next }),
    [dispatch, selection.projectId]
  );
  const replace = useCallback(
    (next: UpscaleWidgetValues) =>
      dispatch({ projectId: selection.projectId, type: 'patchWidgetValues', values: { ...next }, widgetId: 'upscale' }),
    [dispatch, selection.projectId]
  );
