import type { ImageIndexStatusEvent } from '@workbench/image-map/events';
...
export const attachImageMapDataRuntime = (): (() => void) => {
  ...
  const detachers = [
    // The backend routes this to the requesting user's room (plus admins),
    // so any receipt means "my map changed".
    socketHub.on('image_map_projection_ready', refreshIfLoaded),
    ...
    socketHub.on('image_index_status', (payload: never) => {
      ...
      imageMapStore.patchSnapshot({
        indexCounts: { embedded: event.embedded, pending: event.pending, total: event.total },
      });
