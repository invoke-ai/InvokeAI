...
import { createCanvasOperationController } from './operationController';
...
describe('createCanvasOperationController', () => {
  it('holds an exclusive edit lease and closes when the engine invalidates it', () => {
    const edits = createCanvasEditGate();
    const controller = createCanvasOperationController({ edits, isGuardCurrent: () => true });
    ...
    expect(edits.tryAcquire({ kind: 'other' })).toBeNull();
    edits.invalidateDocument();
    expect(controller.getSnapshot()).toEqual({ status: 'idle' });
    expect(edits.tryAcquire({ kind: 'other' })?.isCurrent()).toBe(true);
  });
