...
const createHarness = (snapshot = visualSnapshot()) => {
  ...
  const operationStores = createCanvasOperationStores();
  operationStores.samSession.set(snapshot);
  const invalidate = vi.fn();
  const updateSamInput = vi.fn((input: SamSessionSnapshot['input']) => {
    const current = operationStores.samSession.get();
    if (current) {
      operationStores.samSession.set({ ...current, input });
    }
  });
  const ctx = {
    getSamInteraction: () => {
      const current = operationStores.samSession.get();
      return current?.input.type === 'visual'
        ? { input: current.input, pointLabel: current.pointLabel, sourceRect: current.sourceRect }
        : null;
    },
    invalidate,
    stores,
  ...
  return { ctx, invalidate, operationStores, stores, tool: createSamTool(), updateSamInput };
};
...
describe('createSamTool', () => {
  it('adds configured points on click and temporarily flips their label with Shift', () => {
    ...
    expect(h.operationStores.samSession.get()?.input).toEqual({
      bbox: null,
  ...
  it('removes an existing point on click but drags it after the screen threshold', () => {
    ...
    expect(h.operationStores.samSession.get()?.input.includePoints).toEqual([]);

    h.operationStores.samSession.set(
      visualSnapshot({ input: { bbox: null, excludePoints: [], includePoints: [{ x: 20, y: 20 }], type: 'visual' } })
    );
    down(h, pointer(20, 20));
    move(h, pointer(21, 21));
    expect(h.operationStores.samSession.get()?.input.includePoints).toEqual([{ x: 20, y: 20 }]);
    move(h, pointer(35, 40));
    up(h, pointer(35, 40, { buttons: 0 }));
    expect(h.operationStores.samSession.get()?.input.includePoints).toEqual([{ x: 35, y: 40 }]);
  });
  ...
  it('creates a clipped bbox from an empty drag and treats a sub-threshold drag as a click', () => {
    ...
    expect(h.operationStores.samSession.get()?.input.bbox).toEqual({ height: 20, width: 20, x: 80, y: 80 });

    down(h, pointer(10, 10));
    move(h, pointer(12, 12));
    up(h, pointer(12, 12, { buttons: 0 }));
    expect(h.operationStores.samSession.get()?.input.includePoints).toEqual([{ x: 12, y: 12 }]);
  });
  ...
  it('does not publish a degenerate bbox when a boundary drag clips to zero area', () => {
    ...
    expect(h.operationStores.samSession.get()?.input.bbox).toBeNull();
  });
  ...
  it('accepts and canonicalizes a near-edge point but rejects the exact right and bottom edges', () => {
    ...
    expect(h.operationStores.samSession.get()?.input.includePoints).toEqual([{ x: 99, y: 99 }]);
  });
  ...
  it('moves the bbox body and resizes a handle while clipping to source bounds', () => {
    ...
    expect(h.operationStores.samSession.get()?.input.bbox).toEqual({ height: 30, width: 40, x: 60, y: 70 });
  ...
  it('restores the exact pre-gesture input on pointercancel', () => {
    ...
    expect(h.operationStores.samSession.get()?.input.excludePoints).toEqual([{ x: 50, y: 50 }]);
    h.tool.onPointerCancel?.(h.ctx);

    expect(h.operationStores.samSession.get()?.input).toEqual(initial.input);
  });
});
