...
describe('createCanvasEngine', () => {
  ...
  it('exportLayerPixels can bake raster adjustments in layer-local space without baking presentation props', async () => {
    ...
    const adjusted = await engine.exportLayerPixels('a', { applyAdjustments: true });
    ...
    if (raw.status === 'ok' && adjusted.status === 'ok') {
      ...
      expect(engine.isLayerExportGuardCurrent(adjusted.guard)).toBe(true);
  ...
  it('exportBakedLayerPixels applies adjustments exactly once when explicitly requested', async () => {
    ...
    expect((await engine.exportBakedLayerPixels('a', { applyAdjustments: true })).status).toBe('ok');
...
describe('commitRasterFilterResult', () => {
  ...
  it('replaces with local paint pixels, clears adjustments, and restores exact state with one history entry', async () => {
    ...
    const before = structuredClone(engine.getDocument()!.layers[0]!);
    const exported = await engine.exportLayerPixels(layer.id);
    if (exported.status !== 'ok') {
    ...
    expect(engine.stores.canUndo.get()).toBe(true);
    const forwardExport = await engine.exportLayerPixels(layer.id);
    expect(forwardExport.status).toBe('ok');
    if (forwardExport.status !== 'ok') {
    ...
    expect(engine.stores.canUndo.get()).toBe(false);
    const undoExport = await engine.exportLayerPixels(layer.id);
    expect(undoExport.status).toBe('ok');
    if (undoExport.status === 'ok') {
      const undoSources = backend.drawSourcesFor(undoExport.surface as StubRasterSurface);
      const beforeSnapshot = backend.surfaceById(undoSources.at(-1)!);
    ...
    expect(engine.stores.canRedo.get()).toBe(false);
    const redoExport = await engine.exportLayerPixels(layer.id);
    expect(redoExport.status).toBe('ok');
    if (redoExport.status === 'ok') {
      expect(backend.drawSourcesFor(redoExport.surface as StubRasterSurface)).toEqual(forwardSources);
    }
  ...
  it('copies filtered local pixels directly above the source and replays one structural history entry', async () => {
    ...
    expect(engine.getDocument()!.layers.map((candidate) => candidate.id)).toEqual([result.layerId, 'source', 'below']);
    expect(engine.getDocument()!.layers.find((candidate) => candidate.id === 'source')).toEqual(sourceBefore);
    const copy = engine.getDocument()!.layers[0]!;
    ...
    engine.undo();
    expect(engine.getDocument()!.layers.map((candidate) => candidate.id)).toEqual(['source', 'below']);
    expect(engine.getDocument()!.selectedLayerId).toBe(below.id);
    expect(engine.stores.canUndo.get()).toBe(false);
    engine.redo();
    expect(engine.getDocument()!.layers.map((candidate) => candidate.id)).toEqual([result.layerId, 'source', 'below']);
    expect(engine.getDocument()!.selectedLayerId).toBe(result.layerId);
    expect(engine.getDocument()!.layers[0]).toEqual(copy);
    expect(engine.stores.canRedo.get()).toBe(false);
    engine.dispose();
  });
  ...
  it.each(['allocation', 'draw'] as const)(
    ...
    async (failure) => {
      ...
      expect(result.status).toBe('committed');
      const expectedDocument = structuredClone(engine.getDocument()!);
      const expectedCache = await engine.exportLayerPixels(source.id);
      ...
      expect(() => engine.undo()).toThrow(`replay cache ${failure} failed`);

      expect(engine.getDocument()).toEqual(expectedDocument);
      expect(engine.stores.canUndo.get()).toBe(true);
      expect(engine.stores.canRedo.get()).toBe(false);
      expect(store.dispatch).not.toHaveBeenCalled();
      expect(bitmapStore.markLayerDirty).not.toHaveBeenCalled();
      ...
      const afterFailure = await engine.exportLayerPixels(source.id);
  ...
  it.each(['allocation', 'draw'] as const)(
    ...
    async (failure) => {
      ...
      expect(result.status).toBe('committed');
      const filteredDocument = structuredClone(engine.getDocument()!);
      engine.undo();
      const expectedDocument = structuredClone(engine.getDocument()!);
      const expectedCache = await engine.exportLayerPixels(source.id);
      if (expectedCache.status !== 'ok') {
      ...
      expect(() => engine.redo()).toThrow(`replay cache ${failure} failed`);
      ...
      engine.redo();
      expect(engine.getDocument()).toEqual(filteredDocument);
      expect(engine.stores.canUndo.get()).toBe(true);
      expect(engine.stores.canRedo.get()).toBe(false);
      unsubscribeThumbnail();
      engine.dispose();
  ...
  it('discards obsolete persistence and keeps the exact durable image ref on replace redo', async () => {
    ...
    expect(bitmapStore.markLayerDirty).not.toHaveBeenCalled();
    const redone = engine.getDocument()!.layers[0];
  ...
  describe('paint persistence cancellation', () => {
    ...
    const createRealBitmapStoreHarness = async (
      ...
      const initial = await engine.exportLayerPixels(source.id);
      ...
      const refreshPlacement = async (layerId = source.id) => {
        const exported = await engine.exportLayerPixels(layerId);
        if (exported.status !== 'ok') {
    ...
    it('cancels pending debounced live-paint persistence before durable replace publication', async () => {
      ...
      await expect(
        harness.engine.commitRasterFilterResult({
      ...
      const committed = structuredClone(harness.engine.getDocument()!);
      ...
      expect(harness.engine.getDocument()).toEqual(committed);
      expect(harness.engine.getDocument()!.layers[0]).toMatchObject({
        source: { bitmap: durableImage, type: 'paint' },
      });
      const cache = await harness.engine.exportLayerPixels(harness.source.id);
      ...
      const reloadDocument = structuredClone(harness.engine.getDocument()!);
      harness.engine.dispose();
    ...
    it('redo discards a deferred upload from the restored unpersisted paint snapshot', async () => {
      ...
      expect(harness.engine.stores.canUndo.get()).toBe(true);
      expect(harness.engine.stores.canRedo.get()).toBe(false);
      const redone = harness.engine.getDocument()!.layers[0];
      expect(redone).toMatchObject({ source: { bitmap: durableImage, type: 'paint' } });
      harness.engine.dispose();
    });
    ...
    it('copy undo generation-cancels a deferred upload before redo reuses the layer id', async () => {
      ...
      const copied = await harness.engine.commitRasterFilterResult({
        guard: harness.exported.guard,
        image: durableImage,
        mode: 'copy',
        rect: harness.exported.rect,
      ...
      const durableCopy = structuredClone(
        harness.engine.getDocument()!.layers.find((layer) => layer.id === copied.layerId)!
      );

      harness.engine.selectAll();
      harness.engine.fillSelection();
      ...
      await harness.refreshPlacement(copied.layerId);
      harness.engine.undo();
      expect((await harness.engine.exportLayerPixels(copied.layerId)).status).toBe('missing');
      harness.engine.redo();
      expect(harness.engine.getDocument()!.layers.find((layer) => layer.id === copied.layerId)).toEqual(durableCopy);
      ...
      expect(harness.engine.getDocument()!.layers.find((layer) => layer.id === copied.layerId)).toEqual(durableCopy);
      expect(harness.engine.stores.canUndo.get()).toBe(true);
      const cache = await harness.engine.exportLayerPixels(copied.layerId);
      expect(cache.status).toBe('ok');
      if (cache.status === 'ok') {
        expect(drawGraphContains(cache.surface as StubRasterSurface, harness.backend, 'bitmap-filtered-result')).toBe(
          true
  ...
  it.each(['allocation', 'draw'] as const)(
    ...
    async (failure) => {
      ...
      const copy = structuredClone(engine.getDocument()!.layers[0]!);
      engine.undo();
      ...
      expect((await engine.exportLayerPixels(result.layerId)).status).toBe('missing');
      expect(engine.stores.canUndo.get()).toBe(false);
      expect(engine.stores.canRedo.get()).toBe(true);
      expect(store.dispatch).not.toHaveBeenCalled();
      expect(bitmapStore.markLayerDirty).not.toHaveBeenCalled();
  ...
  it('returns stale without mutation after the source cache version changes', async () => {
    const harness = await createPendingGuardHarness();
    await harness.engine.clearCaches();
    harness.decoded.resolve(new Blob());
  ...
  it.each(['replace', 'copy'] as const)(
    ...
    async (mode) => {
      ...
      const afterExport = await engine.exportLayerPixels(source.id);
  ...
  it('refreshes the mirror and renders the filtered result when an earlier document observer interrupts dispatch', async () => {
    ...
    expect(engine.getDocument()).toBe(reducerDocument);
    expect(engine.stores.canUndo.get()).toBe(true);
    const committedExport = await engine.exportLayerPixels(source.id);
  ...
  it('keeps copy undo retryable when removal fails after restoring the prior selection', async () => {
    ...
    expect(() => engine.undo()).toThrow('copy removal failed');
...
describe('guarded filter previews', () => {
  ...
  it('does not replace a different active canvas operation with a recommendation', async () => {
    ...
    expect(engine.stores.samSession.get()?.sourceRect).toEqual({ height: 10, width: 10, x: 0, y: 0 });
    engine.dispose();
  });
...
describe('Select Object canvas engine integration', () => {
  ...
  it('keeps Select Object visible and inputs intact when the first Process fails to upload, then retries', async () => {
    ...
    expect(engine.startSelectObject('source')).toBe('started');
  ...
  const invalidateSelectObjectCommit = async (
    ...
    if (kind === 'input') {
      engine.updateSelectObjectSession({ input: { prompt: 'new input', type: 'prompt' } });
    } else if (kind === 'reset') {
      engine.resetSelectObjectSession();
    } else {
      await engine.processSelectObjectSession();
    }
  };
  ...
  it('resets after a first failed Process while preserving lifecycle ownership', async () => {
    ...
    await expect(engine.processSelectObjectSession()).resolves.toBe('error');
    expect(engine.stores.samSession.get()).toMatchObject({ error: { code: 'queue' }, status: 'error' });
  ...
  it('keeps the selection when starting on the selected layer and selects a non-selected source', async () => {
    ...
    expect(h.engine.startSelectObject('first')).toBe('started');
  ...
  it('runs a transformed layer rect through upload, local visual graph, metadata, and decoded rect', async () => {
    ...
    expect(h.engine.startSelectObject('scaled')).toBe('started');
    expect(h.engine.stores.samSession.get()?.sourceRect).toEqual({ height: 20, width: 30, x: -6, y: 9 });
    h.engine.updateSelectObjectSession({
      input: {
  ...
  it('saves the guarded preview to the selection mask and exits only after success', async () => {
    ...
    await expect(h.engine.saveSelectObjectSession('selection', makeDurable)).resolves.toEqual({ status: 'selected' });
  ...
  it('Apply replaces the source layer content in place with one undoable history entry', async () => {
    ...
    await expect(h.engine.applySelectObjectSession(makeDurable)).resolves.toEqual({
      layerId: 'source',
      status: 'committed',
    });
    ...
    const replaced = h.engine.getDocument()!.layers.find((layer) => layer.id === 'source')!;
    ...
    expect(h.engine.stores.canUndo.get()).toBe(false);
    h.engine.redo();
    expect(h.engine.getDocument()!.layers.find((layer) => layer.id === 'source')).toEqual(replaced);
    h.engine.dispose();
  });
  ...
  it('blocks Select Object actions called directly after an external lock and still allows Cancel', async () => {
    ...
    await expect(h.engine.applySelectObjectSession(makeDurable)).resolves.toEqual({ status: 'locked' });
  ...
  it('blocks Select Object apply mutation when an external lock begins during final decode', async () => {
    ...
    const pending = h.engine.applySelectObjectSession(() => Promise.resolve());
  ...
  it('interrupts Select Object processing on an external lock without losing inputs or operation', async () => {
    ...
    expect(h.engine.stores.samSession.get()?.status).toBe('preparing-source');
    await vi.waitFor(() => expect(runGraph).toHaveBeenCalledOnce());
  ...
  it('does not invalidate a pending save when preview isolation changes', async () => {
    const h = await createCommittingSamHarness();
    const durable = createDeferred<void>();
    const pending = h.engine.saveSelectObjectSession('inpaint_mask', () => durable.promise);

    h.engine.updateSelectObjectSession({ isolatedPreview: false });
    durable.resolve();

    await expect(pending).resolves.toMatchObject({ status: 'committed' });
    expect(h.engine.getDocument()?.layers[0]?.type).toBe('inpaint_mask');
    h.engine.dispose();
  });
  ...
  it('keeps the session open across saves so a second save also succeeds without touching the source', async () => {
    ...
    expect(h.engine.getDocument()?.layers.map((layer) => layer.name)).toEqual([
      'Segmented Object',
      'Segmented Object',
      'source',
    ]);
    expect(h.engine.getDocument()?.layers[2]).toEqual(h.document.layers[0]);
    expect(h.engine.stores.samSession.get()).toMatchObject({ hasPreview: true, status: 'ready' });
    expect(h.engine.stores.activeTool.get()).toBe('sam');
    h.engine.dispose();
  });
  ...
  it('publishes a committing phase and mutually excludes Apply and Save', async () => {
    ...
    expect(h.engine.stores.samSession.get()?.status).toBe('committing');
    await expect(h.engine.applySelectObjectSession(() => Promise.resolve())).resolves.toEqual({ status: 'busy' });
    await expect(h.engine.saveSelectObjectSession('control', () => Promise.resolve())).resolves.toEqual({
      status: 'busy',
    });
    durable.resolve();
    await expect(pending).resolves.toMatchObject({ status: 'committed' });
  ...
  it.each([
    ...
    const pending = h.engine.saveSelectObjectSession(target, () => Promise.resolve());
    await vi.waitFor(() => expect(resolutions).toBe(2));
  ...
  it('does not close a replacement session when the old selection save finishes authoritatively', async () => {
    ...
    await expect(engine.saveSelectObjectSession('selection', () => Promise.resolve())).resolves.toEqual({
      status: 'selected',
    });
  ...
  it.each(['apply', 'save'] as const)(
    ...
    async (kind) => {
      ...
      const result =
        kind === 'apply'
          ? await engine.applySelectObjectSession(() => Promise.resolve())
  ...
  it('survives bbox moves, layer reorders, and other-layer edits but cancels on a source-layer edit', async () => {
    ...
    const reordered = h.engine.getDocument()!;
    h.setDocument({ ...reordered, layers: [...reordered.layers].reverse() });
    expect(h.engine.stores.samSession.get()).not.toBeNull();

    const withOtherEdited = h.engine.getDocument()!;
    h.setDocument({
      ...withOtherEdited,
    ...
    const withSourceEdited = h.engine.getDocument()!;
    h.setDocument({
      ...withSourceEdited,
  ...
  it('does not start from a stale cache whose replacement source is still decoding', async () => {
    ...
    expect(h.engine.startSelectObject('moving-source')).toBe('not-ready');
    expect(h.engine.stores.samSession.get()).toBeNull();
    expect(h.engine.canvasOperations.getSnapshot()).toEqual({ status: 'idle' });

    nextImage.resolve(new Blob(['inside']));
    await expect(h.engine.exportLayerPixels('moving-source')).resolves.toMatchObject({ status: 'ok' });
    expect(h.engine.startSelectObject('moving-source')).toBe('started');
    h.engine.dispose();
  });
  ...
  it('refuses disabled and authoritatively empty source layers without rasterizing', async () => {
    ...
    expect(h.engine.startSelectObject('disabled')).toBe('disabled');
    expect(h.engine.startSelectObject('empty')).toBe('not-ready');
  ...
  it('validates the source layer on start: missing, unsupported, disabled, and locked', async () => {
    ...
    expect(h.engine.startSelectObject('missing')).toBe('missing');
    expect(h.engine.startSelectObject('mask')).toBe('unsupported');
    expect(h.engine.startSelectObject('disabled')).toBe('disabled');
    expect(h.engine.startSelectObject('locked')).toBe('locked');

    expect(h.engine.stores.samSession.get()).toBeNull();
    expect(h.engine.canvasOperations.getSnapshot()).toEqual({ status: 'idle' });
    expect(h.engine.stores.activeTool.get()).toBe('view');

    expect(h.engine.startSelectObject('ready')).toBe('started');
    expect(h.engine.stores.activeTool.get()).toBe('sam');
    h.engine.dispose();
  });
  ...
  it('isolates the transformed source layer with display semantics while excluding other layers', async () => {
    ...
    h.engine.stores.checkerboard.set(false);
    expect(h.engine.startSelectObject('adjusted-raster')).toBe('started');
    expect(h.engine.stores.samSession.get()?.sourceRect).toEqual({ height: 15, width: 20, x: 4, y: 17 });
    h.engine.updateSelectObjectSession({
  ...
  it('encodes exactly the baked source layer surface, excluding other layers and previews', async () => {
    ...
    await flushMicrotasks();
    expect(h.engine.startSelectObject('raster')).toBe('started');
    h.engine.updateSelectObjectSession({
  ...
  it('lets Escape cancel a gesture before a later Escape cancels the operation', async () => {
    ...
    h.engine.handleEscapePriority({ gestureWasActive: true });
    expect(h.engine.stores.samSession.get()).not.toBeNull();
