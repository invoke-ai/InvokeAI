...
// Expose the built handler and the completed-invocation tracking map that setEventListeners hands
// to both the coordinator and the gallery handler, so tests can assert neither is touched.
 ...
 * client's local execution/progress state. The filtering has to happen at the socket listener,
 * before the workflow execution coordinator records the event, so these tests drive the real
 * listeners + real coordinator rather than calling a downstream handler directly.
 */
describe('setEventListeners cross-user isolation', () => {
  ...
  it.each(['invocation_started', 'invocation_progress', 'invocation_error', 'invocation_complete'])(
    "ignores another user's %s for an authenticated admin",
    (eventName) => {
