...
describe('Wan low-noise partner picker', () => {
  it('offers an untagged single-file A14B', () => {
    ...
    // heuristic and is absent from `ModelRecordChanges`. Reidentify can re-probe the
    // record, but the unchanged untagged filename yields `none` again — so requiring
    // `expert === 'low'` here strands the pair until the file itself is renamed.
...
describe('primary main model options', () => {
  it('keeps MiniMax H3 single-file transformers in their dedicated advanced slot', () => {
    const diffusers = {
      key: 'minimax-diffusers',
      type: 'main',
      base: 'minimax-h3',
      format: 'diffusers',
      name: 'minimax-diffusers',
    } as unknown as MainModelConfig;
    const checkpoint = {
      ...
      base: 'minimax-h3',
      format: 'checkpoint',
      name: 'minimax-checkpoint',
    } as unknown as MainModelConfig;

    expect(selectPrimaryMainModelOptions([diffusers, checkpoint]).map((config) => config.key)).toEqual([
      'minimax-diffusers',
    ]);
  });
});
