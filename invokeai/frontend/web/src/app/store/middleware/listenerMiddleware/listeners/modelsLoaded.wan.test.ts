...
const minimaxH3Checkpoint = {
  key: 'minimax-h3-checkpoint',
  hash: 'h',
  name: 'MiniMax H3 Transformer',
  base: 'minimax-h3',
  type: 'main',
  format: 'checkpoint',
} as unknown as AnyModelConfig;

const minimaxH3Diffusers = {
  ...minimaxH3Checkpoint,
  key: 'minimax-h3-diffusers',
  name: 'MiniMax H3',
  format: 'diffusers',
} as unknown as AnyModelConfig;
...
describe('handleMainModels — MiniMax H3 transformer overrides', () => {
  it('does not auto-select a transformer-only checkpoint when it is the only main model', () => {
    const dispatch = vi.fn();
    handleMainModels([minimaxH3Checkpoint], makeState(), dispatch, log);

    expect(dispatch).not.toHaveBeenCalled();
  });

  it('replaces a stale transformer-only primary with an available Diffusers main', () => {
    const dispatch = vi.fn();
    const state = { params: { model: minimaxH3Checkpoint } } as unknown as RootState;
    handleMainModels([minimaxH3Checkpoint, minimaxH3Diffusers], state, dispatch, log);

    expect(dispatch).toHaveBeenCalledTimes(1);
    expect(dispatch).toHaveBeenCalledWith(modelSelected(minimaxH3Diffusers));
  });
});
