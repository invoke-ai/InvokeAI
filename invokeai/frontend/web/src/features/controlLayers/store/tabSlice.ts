    ...
    builder.addMatcher(isTabInstanceParamsAction, (state, action) => {
      ...
      switch (context.tab) {
        ...
        case 'upscaling':
          ...
          break;
        case 'video':
          state.video = tabInstanceParamsSlice.reducer(state.video, action);
          break;
      }
    });
  },
...
type SerializedTabState = Omit<TabState, 'generate' | 'upscaling' | 'video'> & {
  generate: SerializedTabInstanceParamsState;
  upscaling: SerializedTabInstanceParamsState;
  video: SerializedTabInstanceParamsState;
};
...
export const tabSliceConfig: SliceConfig<typeof tabSlice, TabState, SerializedTabState> = {
  ...
  persistConfig: {
    ...
    serialize: (state) => ({
      ...
      video: {
        ...state.video,
        refImages: omit(state.video.refImages, refImagesDenyList),
      },
    }),
    deserialize: (state) => {
