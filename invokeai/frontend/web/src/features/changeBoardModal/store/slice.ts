 ...
 *   the earlier request's failures are written in under the newer operation's identity. The
 *   owning component reopens the modal only after this check accepts a retained failure, so the
 *   operation ID still protects the reachable retry path.
