    ...
    def lock(self, cache_entry: CacheRecord) -> None:
        ...
        if cache_entry.key not in self._cached_models:
            self._logger.info(
                ...
                f"(Type: {cache_entry.cached_model.model.__class__.__name__}), "
                "but it has already been dropped from the RAM cache. This is a sign that the model loading order "
                "is non-optimal in the invocation code."
            )
        # cache_entry = self._cached_models[key]
        cache_entry.lock()
    ...
    def unlock(self, cache_entry: CacheRecord) -> None:
        ...
        if cache_entry.key not in self._cached_models:
            self._logger.info(
                ...
                f"(Type: {cache_entry.cached_model.model.__class__.__name__}), "
                "but it has already been dropped from the RAM cache. This is a sign that the model loading order is "
                "non-optimal in the invocation code."
            )
        # cache_entry = self._cached_models[key]
        cache_entry.unlock()
        self._logger.debug(
