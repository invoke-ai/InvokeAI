...
class CacheRecord:
    ...
    is_stale: bool = False
    # Post-admission grace: set by ModelCache.put() for unclaimed, non-prefetch admissions and
    # cleared on the entry's first lock(). A freshly admitted model is about to be used - its
    # loader calls get() as soon as put() returns, then
    ...
    # releasers: an unclaimed admission made with no deferred worker running, and one made after
    # shutdown() - after which no further put() is guaranteed to run the sweep. Claimed admissions
    # use admission_claim_ref below instead, including when no worker can carry a hold. For the same
    # reason shutdown() evicts a record whose only shield is this flag rather than stale-retaining
    # it: a load cancelled before its retrieval leaves no wrapper, so nothing but that sweep could
    # ever have released it. From the retrieval on, the window is
    ...
    #
    # Only the asynchronous sweeps consult it, through in_first_use_window below. Synchronous
    # paths ignore this unowned grace, but honor admission_claim_ref because a live claimed load
    # has no other safe fallback after worker recovery.
    ...
    def in_first_use_window(self) -> bool:
        ...
        it has not come back to retrieve it (admission_in_flight). The asynchronous eviction sweeps
        (budget reconcile and peer-requested eviction) treat such a record like a locked one:
        evicting it would detach a record whose holder is about to lock it, splitting the model from
        the cache's RAM accounting and - for shared weights - releasing store ownership while the
        tensors live on, so a peer's reload would mint a duplicate canonical copy. shutdown() honors
        the owned portions of this window (first_use_holds and admission_in_flight) but deliberately
        does not retain the unowned awaiting_first_use grace. The synchronous paths (make_room,
        drop_model, unlock's stale eviction) honor first_use_holds and admission_claim_ref, but
        ignore the unowned awaiting_first_use grace."""
