...
def test_pid_optimization_defaults_to_false_and_loads_from_yaml(tmp_path: Path, patch_rootdir: None) -> None:
    assert InvokeAIAppConfig().pid_optimization is False

    temp_config_file = tmp_path / "temp_invokeai.yaml"
    temp_config_file.write_text('schema_version: "4.0.3"\npid_optimization: true\n')

    assert load_and_migrate_config(temp_config_file).pid_optimization is True
