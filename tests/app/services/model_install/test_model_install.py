...
def test_lora_metadata_import(mm2_installer: ModelInstallServiceBase, tmp_path: Path) -> None:
    """Test that LoRA metadata from JSON files is properly imported."""
    import json
    
    # Create a fake LoRA model file 
    ...
    metadata = {
        ...
        "activation text": "test_trigger, secondary_trigger",
        "sd version": "SDXL",
        "preferred weight": 0.8,
        "negative text": "bad quality"
    }
    
    json_path = tmp_path / "test_lora.json"
    with open(json_path, 'w') as f:
        json.dump(metadata, f)
    
    # Create a PNG file as well
    png_path = tmp_path / "test_lora.png"
    png_path.write_bytes(b"fake png data")
    
    # Override the _probe method to return a LoRA config for our fake file
    from invokeai.backend.model_manager.config import LoRADiffusersConfig, BaseModelType, ModelFormat
    
    original_probe = mm2_installer._probe
    
    def mock_probe(model_path, config=None):
        if model_path.name == "test_lora.safetensors":
            return LoRADiffusersConfig(
                key="test_lora_key",
                hash="fake_hash",
                path=str(model_path),
                file_size=len(b"fake lora data"),
                name="test_lora",
        ...
        return original_probe(model_path, config)
    
    mm2_installer._probe = mock_probe
    
    try:
        # Register the model
        key = mm2_installer.register_path(lora_path)
        
        # Get the registered model
        model_config = mm2_installer.record_store.get_model(key)
        
        # Verify that metadata was applied
        assert model_config.description == "Test LoRA description | Additional notes"
        assert model_config.trigger_phrases == {"test_trigger", "secondary_trigger"}
        
    finally:
        # Restore the original probe method
        mm2_installer._probe = original_probe
