...
class Installer:
    ...
    def mktemp_venv(self) -> TemporaryDirectory:
        ...
        if OS == "Windows" and int(platform.python_version_tuple()[1]) >= 10:
            venv_dir = TemporaryDirectory(
                prefix=BOOTSTRAP_VENV_PREFIX, ignore_cleanup_errors=True
            )
        else:
    ...
    def app_venv(self, path: str = None):
        ...
        # experimental / testing
            ...
            venv_dir = (
                Path(venv_dir_parent).expanduser().resolve()
                / f"InvokeAI/{VERSION}/venv"
            )
    ...
    def install(
        ...
        self.dest = (
            ...
            if yes_to_all
            else messages.dest_path(root)
        )

        # create the venv for the app
        self.venv = self.app_venv()

        self.instance = InvokeAiInstance(
            runtime=self.dest, venv=self.venv, version=version
        )

        # install dependencies and the InvokeAI application
        (extra_index_url, optional_modules) = (
            get_torch_source() if not yes_to_all else (None, None)
        )
...
def set_sys_path(venv_path: Path) -> None:
    ...
    lib = (
        "Lib"
        if OS == "Windows"
        else f"lib/python{sys.version_info.major}.{sys.version_info.minor}"
    )
