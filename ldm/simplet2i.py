...
"""
    ...
    def _img2img(
        ...
        init_latent_set = None
        for img in init_img:
            init_image = self._load_img(img,width,height).to(self.device)
            with precision_scope(self.device.type):
                init_latent = self.model.get_first_stage_encoding(
                    self.model.encode_first_stage(init_image)
                )  # move to latent space
            if init_latent_set is None:
                init_latent_set = init_latent
            else:
                init_latent_set = torch.cat((init_latent_set, init_latent), 0)

        if init_latent_set.shape[0] == 1:
            # Single image, use old behavior
            init_latent = init_latent_set
            latent_noise = None
            weight = None
        else:
        ...
        def make_image(x_T):
            ...
            if weight is not None:
                latent_noise = torch.randn_like(weight)
                latent_noise = torch.mul(weight, latent_noise)
    ...
    def _load_img_set(self, path):
        with open(path) as f:
            img_files = [line.rstrip() for line in f]
