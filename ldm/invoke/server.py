...
def build_opt(post_data, seed, gfpgan_model_exists):
    ...
    setattr(
        opt,
        "facetool_strength",
        float(post_data["facetool_strength"]) if gfpgan_model_exists else 0,
    )
    setattr(
        opt,
        "upscale",
    ...
    setattr(
        opt,
        "variation_amount",
        float(post_data["variation_amount"]) if int(post_data["seed"]) != -1 else 0,
    )
    ...
    def do_POST(self):
                    ...
                    self.wfile.write(
                        bytes(
                            json.dumps(
                                {
                                    "event": action,
                                    "processed_file_cnt": f"{x}/{opt.iterations}",
                                }
                            )
                            + "\n",
                            "utf-8",
        ...
        def image_progress(sample, step):
            ...
            if (wants_progress_image | wants_progress_latents) and step < opt.steps - 1:
                image = (
                    self.model.sample_to_image(sample)
                    if wants_progress_image
            ...
            self.wfile.write(
                bytes(
                    json.dumps({"event": "step", "step": step + 1, "url": path}) + "\n",
                    "utf-8",
                )
            )
