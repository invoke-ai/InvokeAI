...
from ldm.invoke.globals import global_cache_dir, global_config_dir
...
def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=False):
    ...
    if sum(k.startswith("model_ema") for k in keys) > 100:
        ...
        if extract_ema:
            ...
            print("  | Extracting only the non-EMA weights (usually better for fine-tuning)")
...
def convert_paint_by_example_checkpoint(checkpoint):
    ...
    # load mapper
    keys_mapper = {
        k[len("cond_stage_model.mapper.res") :]: v for k, v in checkpoint.items() if k.startswith("cond_stage_model.mapper")
    }
...
def load_pipeline_from_original_stable_diffusion_ckpt(
    ...
    with warnings.catch_warnings():
        ...
        checkpoint = (
            load_file(checkpoint_path) if Path(checkpoint_path).suffix == ".safetensors" else torch.load(checkpoint_path)
        )
        cache_dir = global_cache_dir("hub")
