#
# fine-tuning of base model via low rank adapation (LoRA), using the PEFT huggingface library
#
# code is based on train_text_to_image_lora.py from this diffusers PR:
#            https://github.com/huggingface/diffusers/pull/2479 from haofanwang
# and originally modified for InvokeAI by genomancer
"""Fine-tuning script for Stable Diffusion for text2image with support for LoRA, using PEFT library."""
...
# removing 0.14 requirement for now, will see if it causes problems...
# check_min_version("0.14.0.dev0")

# verified working with 0.13.1
check_min_version("0.13")
...
def main():
    ...
    with accelerator.main_process_first():
        if args.max_train_samples is not None:
            if args.seed is not None:
                ...
                dataset["train"] = dataset["train"].shuffle().select(range(args.max_train_samples))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        # commenting out validation tests for now
        # if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        if args.use_peft:
            ...
            if args.train_text_encoder:
                print("TRAIN_TEXT_ENCODER: also applying LoRA training to text encoder")
    ...
    for _ in range(args.num_validation_images):
        if args.seed is not None:
            ...
            generator = torch.Generator(device=accelerator.device).manual_seed(_)
            print("Seed: " + str(_))
        image = pipeline(args.validation_prompt, num_inference_steps=30, generator=generator).images[0]
        image.save(args.output_dir + args.instance_prompt + "_" + str(_) + ".png")
        # images.append(image)
