...
class TextualInversionManager(BaseTextualInversionManager):
    ...
    def load_textual_inversion(self, ckpt_path: Union[str, Path], defer_injecting_tokens: bool = False):
        ...
        try:
            ...
            print(f"### {ckpt_path.parents[0].name}/{ckpt_path.name} is damaged or corrupt.")
            return
        ...
        if embedding_info is None:
            ...
            return
        elif self.text_encoder.get_input_embeddings().weight.data[0].shape[0] != embedding_info["token_dim"]:
            print(
        ...
        sourcefile = f"{ckpt_path.parent.name}/{ckpt_path.name}" if ckpt_path.name == "learned_embeds.bin" else ckpt_path.name

        if trigger_str in self.trigger_to_sourcefile:
            replacement_trigger_str = (
                f"<{ckpt_path.parent.name}>" if ckpt_path.name == "learned_embeds.bin" else f"<{ckpt_path.stem}>"
            )
            print(
    ...
    def _add_textual_inversion(self, trigger_str, embedding, defer_injecting_tokens=False) -> Optional[TextualInversion]:
        """
    ...
    def _inject_tokens_and_assign_embeddings(self, ti: TextualInversion) -> int:
        ...
        if ti.embedding_vector_length > 1:
            ...
            pad_token_strings = [
                ti.trigger_string + "-!pad-" + str(pad_index) for pad_index in range(1, ti.embedding_vector_length)
            ]
            # todo: batched UI for faster loading when vector length >2
            pad_token_ids = [
                self._get_or_create_token_id_and_assign_embedding(pad_token_str, ti.embedding[1 + i])
    ...
    def _parse_embedding_bin(self, embedding_file):
        ...
        if list(embedding_ckpt.keys()) == 0:
            ...
            for token in list(embedding_ckpt.keys()):
                embedding_info["name"] = token or f"<{os.path.basename(os.path.splitext(embedding_file)[0])}>"
    ...
    def _handle_broken_pt_variants(self, embedding_ckpt: dict, embedding_file: str) -> dict:
        ...
        if isinstance(list(embedding_ckpt["string_to_token"].values())[0], torch.Tensor):
            for token in list(embedding_ckpt["string_to_token"].keys()):
                embedding_info["name"] = token if token != "*" else f"<{os.path.basename(os.path.splitext(embedding_file)[0])}>"
