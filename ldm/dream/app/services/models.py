...
class DreamBase:
    ...
    strength: float = (
        0  # TODO: name this something related to img2img to make it clearer?
    )
    ...
    def parse_json(self, j, new_instance=False):
        ...
        if self.enable_generate:
            ...
            self.progress_images = "progress_images" in j and bool(
                j.get("progress_images")
            )
        ...
        if self.enable_upscale:
            ...
            self.upscale = (
                None
                if self.upscale_level in {None, ""}
                else [int(self.upscale_level), float(self.upscale_strength)]
            )
        ...
        self.time = (
            int(j.get("time"))
            if ("time" in j and not new_instance)
...
class Signal:
    ...
    def image_progress(
        ...
        step: int,
        totalSteps: int,
        progressType: ProgressType = ProgressType.GENERATION,
        hasProgressImage: bool = False,
    ):
        return Signal(
            "dream_progress",
            {
    ...
    def job_started(jobId: str):
        return Signal("job_started", {"jobId": jobId}, job=jobId, broadcast=True)

    @staticmethod
    def job_done(jobId: str):
        return Signal("job_done", {"jobId": jobId}, job=jobId, broadcast=True)

    @staticmethod
    def job_canceled(jobId: str):
        return Signal("job_canceled", {"jobId": jobId}, job=jobId, broadcast=True)
...
class PaginatedItems:
    ...
    def __init__(
        self, items: List[Any], page: int, pages: int, per_page: int, total: int
    ):
        self.items = items
        self.page = page
        self.pages = pages
