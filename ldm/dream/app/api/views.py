...
class ApiImages(MethodView):
    ...
    def __init__(
        self,
        pathBase,
        storage: ImageStorageService = Provide[
            Container.storage_package.image_storage_service
        ],
    ):
        self.__pathRoot = os.path.abspath(
            os.path.join(os.path.dirname(__file__), pathBase)
        )
...
class ApiIntermediates(MethodView):
    init_every_request = False
    __pathRoot = None
    __storage: ImageStorageService = Provide[
        Container.storage_package.image_intermediates_storage_service
    ]
