    ...
    while not done:
        ...
        try:
            ...
            if opt.grid and len(job.results) > 0:
                ...
                formatted_dream_prompt  = opt.dream_prompt_str(seed=first_seed,grid=True,iterations=len(job.results))
                formatted_dream_prompt += f' # {grid_seeds}'
                metadata = metadata_dumps( # TODO: standardize metadata with model class
                    opt,
                    seeds      = grid_seeds,
                ...
                # TODO: Make sure opt.save_original cleans up non-final results here
                results = list(map(lambda dreamResult: [
